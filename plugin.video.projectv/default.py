...
addon = xbmcaddon.Addon('plugin.video.projectv')
...
SOURCES = [{"url": "http://projectv.pw/ProjecTV/MList/ProjecListM.xml", "fanart": "http://nube.projectv.pw/img/gracias.jpg"}]
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.video.projectv-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Projectv-TV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Projectv-TV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Projectv-TV,New source added.,5000,"+icon+")")
        if not url is None:
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Projectv-TV,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(Projectv-TV,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Projectv-TV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Projectv-TV', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from Projectv-TV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('Add to Projectv-TV Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(Projectv-TV,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Projectv-TV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Projectv-TV Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(Projectv-TV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Projectv-TV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
